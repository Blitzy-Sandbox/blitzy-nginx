...
ngx_http_status_def_t` array in `src/http/ngx_http_status.c`, read-only after
initialization, looked up in O(1) by class-offset indexing, and under 1 KB per
worker. Because it lives in `.rodata` it is shared across workers via
copy-on-write — there is no per-worker copy and no shared-memory migration, so
graceful binary upgrade is safe.
...
| **Deck dependency: pin Mermaid to 11.15.0 (security update from the AAP's literal 11.4.0 pin)** *(deviation, recorded per Rule 2)* | Keep the AAP-pinned Mermaid 11.4.0; or drop Mermaid diagrams from the deck. | A post-AAP security review flagged 11.4.0 for CVE-2025-54881 (KaTeX-label XSS, fixed 11.10.0) and CVE-2026-41149 (state-diagram `classDef` DOM injection, fixed 11.15.0). 11.15.0 clears both while fully preserving the AAP's functional intent — a Mermaid-powered deck on the same 11.x major with stable `initialize()`/`run()` APIs. The change is confined to the deck deliverable; no production nginx code is touched. | Practical exploit risk was already near-nil (static, internally-authored deck with no untrusted diagram input). *Mitigation:* exact immutable version pin; all six diagrams re-verified rendering under 11.15.0 in a browser. This is the only deviation from the AAP's literal CDN pin and is recorded here per the Explainability rule. |
