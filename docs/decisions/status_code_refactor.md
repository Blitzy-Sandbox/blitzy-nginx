...
| **Coerce a validation-rejected `err_status` to a clean 500 at the *late* header choke point `ngx_http_send_header()` instead of `return NGX_HTTP_INTERNAL_SERVER_ERROR;` (resolves QA Final finding #1)** — when opt-in strict validation rejects a locally-generated `r->err_status` (a bare `return <unregistered-code>;`, Pattern C, e.g. 451/460/520/599), `ngx_http_send_header()` now logs `invalid status code: %ui` and sets `r->headers_out.status = NGX_HTTP_INTERNAL_SERVER_ERROR`, then falls through to the header filter (the existing `status_line.len = 0;` runs in both branches). | (a) Keep the literal `return NGX_HTTP_INTERNAL_SERVER_ERROR;` form the per-file prompt specified for *both* `ngx_http_core_module.c` sites; (b) document the empty connection close as the intended strict-mode outcome for unmodelled codes. | `ngx_http_send_header()` is the **late** choke point: by the time the special-response flow reaches it, the response is already committed to `err_status` with its (empty) error body laid out, so a `return` there aborts header emission and closes the connection with **zero bytes** — the client observes HTTP&nbsp;000, not a status line. The **earlier** `ngx_http_send_response()` choke point can still `return` 500 because it runs *before* the response is committed, so it is left unchanged. Coercion makes the late path deterministically yield the **clean 500** that the migration guide and AAP §0.7.5 already document as the failure response — honoring the higher-precedence AAP intent over the per-file prompt's literal `return` form (D1). The detection-by-logging purpose (the AAP's stated goal for validation) is fully preserved: `invalid status code: N` still fires for every rejected code. | Risk: none to the **default** build — validation is opt-in (`NGX_HTTP_STATUS_VALIDATION`); with it compiled out `ngx_http_status_set()` never returns non-`NGX_OK`, so the coercion branch is unreachable dead code and the wire behavior is **byte-identical to stock nginx**. **Verified at runtime:** default build emits 451→451, 460→460, 520→520, 599→599 (unchanged, zero validation logs); validation build now emits a clean `HTTP/1.1 500` (Content-Length: 0) for all four (was HTTP&nbsp;000) with the `invalid status code` log still firing, while registered code 418 still emits 418 and `return 451 "body";` / `error_page` paths remain a clean 500. |
