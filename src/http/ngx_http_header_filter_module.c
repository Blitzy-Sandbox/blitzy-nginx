...
ngx_http_header_filter(ngx_http_request_t *r)
{
    ...
    if (r->headers_out.status_line.len) {
        ...
        /*
         * Preserve the per-status-class normalization side-effects that
         * previously lived alongside the offset-indexed reason lookup.
         * 204 and 304 are unique numeric values, so testing them directly
         * is equivalent to the former range+equality checks.
         */
        ...
        /* delegate default reason-phrase resolution to the status registry */

        status_line = ngx_http_status_reason(status);

        if (status_line && status_line->len) {
            len += status_line->len;
