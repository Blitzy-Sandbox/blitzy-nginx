...
- [ ] `ngx_http_core_error_page()` parser at lines 4914–5029 NOT modified (range 300–599, 499 rejection, 494/495/496/497→400 mapping all preserved)
...
- [ ] `src/http/ngx_http_status.c` registry contains all 43 status-code `NGX_HTTP_*` macros plus the 2 `NGX_HTTPS_*` extensions from `ngx_http_request.h` lines 74–145, plus registry-only entries (e.g., 203) — 59 entries total
...
- [ ] All 43 `#define NGX_HTTP_*` status-code macros plus the 2 `NGX_HTTPS_*` extensions in `ngx_http_request.h` lines 74–145 UNCHANGED
- [ ] All 15 in-scope direct `headers_out.status = ...` assignments across 12 files converted to `ngx_http_status_set()` (the pre-refactor empirical census of 33 assignments across 20 files in AAP §0.2.1 includes upstream pass-through and OUT-OF-SCOPE filter-chain sites preserved per D-006 and D-010)
...
- **Decision**: PENDING
- **Rationale**: (to be filled after the Backend Architecture Expert Agent verifies registry completeness against the 45-constant status macro inventory — 43 `NGX_HTTP_*` plus 2 `NGX_HTTPS_*` — confirms each API function's line budget, and audits every one of the 15 in-scope conversion sites for correctness)
...
- [ ] All 59 registered status codes listed with RFC 9110 §15 references (45 macro-backed + registry-only entries)
...
| §0.1.1 G2 | Centralize assignment | 15 in-scope conversions across 12 files (delivered); 33 across 20 was the AAP §0.2.1 pre-refactor empirical census including upstream pass-through and OUT-OF-SCOPE filter-chain sites preserved per D-006/D-010 | (verify) |
...
| §0.1.1 I8 | error_page parser untouched | ngx_http_core_module.c lines 4914-5029 (function `ngx_http_core_error_page`) byte-equal | (verify) |
